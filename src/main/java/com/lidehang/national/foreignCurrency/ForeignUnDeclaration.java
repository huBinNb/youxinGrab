...
/**
 * 外汇 涉外收入申报表--已申报（待审核）信息查询
 * 
 * @author Hobn
 *
 */
public class ForeignUnDeclaration {
	// 已申报（待审核）信息查询
	public String getDeclareQuota(HttpClient httpclient, String organizationCode) {
		String url = "http://bopcom.safesvc.gov.cnBizforCustomerWeb/servlet/raAuditingSearch?current_appCode=BZCN&asone_addr=asone.safesvc.gov.cn:80";
		...
		new CompanyDataDaoImpl().addData(organizationCode, "15003", list);
		return null;
	}

	/**
	 * 已申报（待审核）信息查询 列表   没有数据
	 * 
	 * @param response
	 * @param httpclient
	 * @return
	...
	private List<org.bson.Document> parseList(HttpClient httpclient, String jumpUrl) {
		...
		String action1 = "http://bopcom.safesvc.gov.cn/BizforCustomerWeb/servlet/raAuditingSearch";
		// 当前时间 年 月
			...
			for (int p = dateInt2; p < changeMonth; p++) {
				...
				for (int i = 0; i < pageNum; i++) {
					...
					for (int k = 1; k < trs.size(); k++) {
						...
						resultMap.put("serialNumber", "15003");
						String index = "15003001";
						for (int j = 0; j < tds.size(); j++) {
							String value = tds.get(j).text();
							resultMap.put(index, value);
							index = String.valueOf(Long.parseLong(index) + 1);
						}
						String reportNO = (String) resultMap.get("15003002");
						list.add(CompanyDataUtil.toDocument(resultMap,
								parseListDetails(index, httpclient, jumpUrl, reportNO)));
					}
				}
	...
	private Map<String, Object> parseListDetails(String index, HttpClient httpclient, String jumpUrl, String reportNO) {
		String url = "http://bopcom.safesvc.gov.cn/BizforCustomerWeb/servlet/raAuditingSearchDetail?rptNO=" + reportNO
				+ "&backUrl=" + jumpUrl;
		String response = TaxConstants.getMes(httpclient, url);
