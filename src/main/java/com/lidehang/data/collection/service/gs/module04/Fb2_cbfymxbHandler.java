...
/**
 *企业所得税和非居民企业所得税(包括季报和年报)--附表二（1）——成本费用明细表
 */
public class Fb2_cbfymxbHandler implements GSModuleBase<GSSiteHandler> {
	...
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		...
		for (int i =1; i < trElements.size(); i++) {
			...
			//http://100.0.0.1:8001/ctais2/wssb/sjcx/print_ndsds_fb2a.jsp?k=2
			...
			boolean flag = true;
			for (Element b : aDyxm) {
				if (b.attr("href").startsWith("print_ndsds_fb2a.jsp")) {
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10038", list);
	...
	private Map<String, Object> parseLssb(Element tr) {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10038001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10038002", StringUtils.StringFormat(tr.child(3).child(0).text()));
		map.put("10038003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10038");
		return map;
	}
