 ...
 * 获取解析存储申报信息 -- 存根联明细
 */
public class CglHandler implements GSModuleBase<GSSiteHandler> {
	
//	@Autowired
//	CompanyDataDao companyDataDao;
	private static Logger logger =Logger.getLogger(CglHandler.class);
	@Override
	public SiteStatus start(GSSiteHandler siteHandler) throws SiteLoginFailedException {
		logger.info("国税--获取解析存储申报信息 -- 存根联明细抓取");
...
//		System.out.println(zzsListHtml);
		...
		for(int i=1;i<trElements.size();i++){
			...
			boolean flag = true ;
			for(Element b:aDyxm){
				if(b.attr("href").startsWith("print_zzs_flzl4.jsp")){
		...
		new CompanyDataDaoImpl().addData(siteHandler.params.getCompanyId(), "10003", list);
	...
	private Map<String,Object> parseLssb(Element tr){
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("10003001", StringUtils.StringFormat(tr.child(2).child(0).text()));
		map.put("10003002",StringUtils.StringFormat( tr.child(3).child(0).text()));
		map.put("10003003", StringUtils.StringFormat(tr.child(4).child(0).text()));
		map.put("serialNumber", "10003");
		return map;
	}
